...
/*
We estimate the NRMSEs of estimates of the following average degree estimators for a specified sample size and specified values of p.
estimation0: naive weighting
estimation1: proposed weighting
*/

int several_p_exp(Graph &G,std::string model,const std::vector<double> &ps,const double samplerate,const int exptime,std::vector<std::vector<double> > &NRMSEs){
...
int main(int argc,char *argv[]){
	...
	several_p_exp(G,model,settings.ps,settings.asm_samplerate,settings.numrun,NRMSEs);

	//write result
	const char *dir = "../data/exp/";
	const std::string expfilepath = std::string(dir) + std::string(readgraph) + "_aved_estimation_" + model + ".txt";
