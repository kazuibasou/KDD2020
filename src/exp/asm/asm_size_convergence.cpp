...
/*
We estimate the NRMSEs of convergence values of the following size estimators for specified values of p.
...
int experiment_convergences(Graph &G,const std::vector<double> &ps,const int exptime,std::vector<std::vector<double> > &NRMSEs){
	...
	for(double p:ps){
		...
		for(i=0;i<exptime;++i){
			...
			for(j=0;j<int(convergences.size());++j){
...
int main(int argc,char *argv[]){
	if(argc != 2){
		printf("Error: Please input following: ./asm_size_convergence (name of target dataset)\n");
		exit(0);
	}
	//read graph data.
