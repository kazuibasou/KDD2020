...
/*
We calculate the relative error of convergence values of the following size estimators. 
...
int main(int argc, char *argv[]){
	if(argc != 1){
		printf("Error: Please input following: ./pokec_size_convergence\n");
		exit(0);
	}
	//read graph data.
	...
	size_convergences(G,LPC,convergences);
	...
	const std::string expfilepath = std::string(dir) + std::string(readgraph) + "_size_convergence.txt";
	FILE *ef = fopen(expfilepath.c_str(),"w");
	if(ef == NULL) {
		printf("Could not open file named %s.\n",expfilepath.c_str());
		return -1;
	}
	fprintf(ef,"Convergence values and the relative errors of each size estimator.\n");
	for(int i=0;i<int(convergences.size());++i){
