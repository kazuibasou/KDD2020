...
/*
We estimate the NRMSEs of estimates of the following size estimators for specified sample sizes. 
estimation0: naive weighting
estimation1: proposed weighting
*/

int several_samplesize_exp(Graph &G,std::string model,const std::vector<double> samplerates,const double ratio_m_r,const int exptime,std::vector<std::vector<double> > &NRMSEs){
	int i,j,k,seed,samplesize;
	double NRMSE;
	double exact_value = double(G.N);
...
int main(int argc,char *argv[]){
	if(argc != 2){
		printf("Error: Please input following: ./pokec_size_estimation (access model: ideal or hidden)\n");
		return -1;
	}
	//read graph data.
	...
	several_samplesize_exp(G,model,settings.pokec_samplerates,settings.ratio_m_r,settings.numrun,NRMSEs);

	//write result
	const char *dir = "../data/exp/";
	const std::string expfilepath = std::string(dir) + std::string(readgraph) + "_size_" + model + ".txt";
	FILE *ef = fopen(expfilepath.c_str(),"w");
	if(ef == NULL) {
		printf("Could not open file named %s.\n",expfilepath.c_str());
		return -1;
	}
	fprintf(ef,"The NRMSEs of estimates of each size estimator for specified sample rates.\n");
